...
subsample.Timeseries.MgCa<-function(tss,timepoints,biowidth.time=0.1,NForam=30)
#Resample a equidistant sampled timeseries (e.g. model result) at the "timepoints" by simulating the foraminifera sampling
#Input:
# tss: pTs object containing the equidistant timeseries, units = kyr
# timepoints: vector with the points in time
# biowidth: width of the averaging window in years
# NForam: Number of foraminifera
#Returns: result=list(time,data)
{

    z<-(-1*biowidth.time):(3*biowidth.time) #width of the transfer function (compromise 4x biowidth contains most of the area of the impulse response
    weights<-ImpulseResponse.BergerHeath(-z,biowidth.time,0) #impulse response in year units; use inverted z
    ...
    index.time<-index.time[index.time>0] #Only use points inside the timeseries
    ...
    index.time0<-(biowidth.time+1) #Index of the weight which corresponds to time 0

    result<-list()
    result$data<-rep(NA,length(timepoints))
    for (i in 1:length(index.time))
    {
       shift<-index.time[i]-index.time0 #Shift of the weights that zero time overlaps with the sampling time

     #Put weights outside of the timeseries to zero
       weights.cut<-weights
       index.weight.shift<-(1:length(weights))+shift #index to simplify the
