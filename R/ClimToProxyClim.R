...
ClimToProxyClim_Rapid <- function(clim.signal,
  ...
  stopifnot(AllEquals(bio.depth))
  stopifnot(AllEquals(sed.acc.rate))
  stopifnot(AllEquals(layer.width))
  if (is.function(habitat.weights)) stop("habitat.weights must be constant for rapid version")
  
  if (is.vector(habitat.weights) == FALSE){
    stopifnot(all(apply(habitat.weights, 1, function(i) {
      isTRUE(all.equal(i, habitat.weights[1,]))
    })))
    }
