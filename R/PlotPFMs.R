...
PlotPFMs <- function(PFMs,
  ...
  if (is.null(colr.palette) == FALSE)
    p <- p + scale_colour_manual("", values = colr.palette, breaks = rev(breaks), labels = levl.labels)
...
#   #set factor level ordering for stage
#
#   PFMs$stage <- factor(PFMs$stage, levels = breaks, ordered = TRUE)
#
#
#
#   p <- ggplot(data = PFMs, aes(x = timepoints / 1000, y = value,
#                                colour = stage, alpha = stage)) +
#     # geom_rug(data = rug.dat, sides = "b", colour = "Darkgrey") +
#     geom_line(aes(group = paste0(stage, replicate))) +
#     theme_bw() +
...
#                                  nrow = 1,
#                                  override.aes = list(alpha = 1))) +
#     labs(x = expression("timepoints [ka]"),
