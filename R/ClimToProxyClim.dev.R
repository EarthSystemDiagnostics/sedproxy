...
#' @param habitat.weights Either the seasonal pattern of productivity for the organism(s)
...
#' @param habitat.wt.args A named list of parameters to be passed to a function named in habitat.weights
...
ClimToProxyClim.dev <- function(clim.signal,
  ...
  if (any(is.function(habitat.weights), (is.numeric(habitat.weights)&length(habitat.weights) == ncol(clim.signal))) == FALSE)
    stop("habitat.weights must be either a vector of weights with length = ncol(clim.signal), or a function.
  ...
  if (is.function(habitat.weights)){
    ...
    habitat.weights.weights <- do.call(FUN, args = c(list(x = clim.signal), habitat.wt.args))
    habitat.weights.weights <- habitat.weights.weights / sum(habitat.weights.weights)
  }

  # Ensure seasonal productivities are weights and matrix
  if (is.numeric(habitat.weights)){
  habitat.weights.weights <- habitat.weights / sum(habitat.weights)
  habitat.weights.weights <- matrix(rep(habitat.weights.weights, nrow(clim.signal)),
  ...
  out <- sapply(1:n.timepoints, function(tp) {
    ...
    habitat.weights.weights <- habitat.weights.weights[sig.window.i.1, , drop = FALSE]
    habitat.weights.weights <- habitat.weights.weights / sum(habitat.weights.weights)
    clim.sig.weights <- bioturb.weights * habitat.weights.weights
    ...
    if (is.infinite(n.samples[tp])) {
      ...
      clim.sig.window.ann <- rowSums(clim.sig.window * habitat.weights.weights)
